...
void ensure_aws_cryptosdk_dec_materials_has_allocated_members(struct aws_cryptosdk_dec_materials *materials) {
...
int decrypt_materials(
    ...
    if (nondet_bool()) {
        struct aws_cryptosdk_dec_materials *temp = malloc(sizeof(struct aws_cryptosdk_dec_materials));
        ensure_aws_cryptosdk_dec_materials_has_allocated_members(temp);
        __CPROVER_assume(aws_cryptosdk_dec_materials_is_valid(temp));
        *output = temp;
...
void aws_cryptosdk_cmm_decrypt_materials_harness() {
    const struct aws_cryptosdk_cmm_vt vtable = { .vt_size                = sizeof(struct aws_cryptosdk_cmm_vt),
                                                 ...
                                                 .decrypt_materials      = nondet_bool() ? NULL : decrypt_materials };
    ...
    struct aws_cryptosdk_dec_materials *output;  // used as ** in the call
    struct aws_cryptosdk_dec_request request;    // precondition: non-null
